...
SinglyLinkList<T>& SinglyLinkList<T>:: operator + (const  SinglyLinkList& rhs) {
                   if (head==0 && rhs.head==0) {
                                               return *this;
                                                        ...
                                                        cout<<"Me is called"<<endl;
                                                      //  DNode<T>* TEMP=head->GetNextPtr();
                                                      // cout<<TEMP->Get();
                                                      SinglyLinkList<T> temp;
                                                      /*  while ( TEMP!=head ) {
                        //                                cout<<"Me is called"<<TEMP->Get()<<endl;
                                                        temp.Insert(TEMP->Get());
                                                        TEMP=TEMP->GetNextPtr();
                                                        } 
                                                       // TEMP=0;
                                                        /*TEMP=rhs.head;
...
void SinglyLinkList<T>::Add(const SinglyLinkList& rhs) {
       ...
       if  (max >= rhs.max) {
        ...
        do
          {
          if (temp->GetPrevPtr()==terminate && (max > rhs.max || max<rhs.max) ) {
                               int_t=temp->Get()+0;
                               }
                               else {
                                    int_t=temp->Get()+sec->Get();
                                    }
          cout<<"temp()"<<temp->Get()<<"sec()"<<sec->Get()<<",";
