...
istream& operator >> (istream& inFile,Shape*& par)
{
 inFile >> par->shape_type;
 cout<<par->shape_type<<endl;
 switch (par->shape_type)
          {
            case 'C':{par = new Circle();break;}
            case 'R':{par = new Rectangle();break;}
            case 'S':{par = new Sphere();break;}
            case 'B':{par = new RectangularBox();break;}
          }
          cout<<"Type Id "<<typeid(*par).name()<<endl;
          /*shape[n++]->GetDetail();*/
          par->GetDetail();
 return inFile;
} 
